...
with tab2:
  ...
  # --- HISTÓRICO DE GASTOS AVULSOS COM NOVA LÓGICA DE FILTRAGEM ---
  ...
  filtro_cat_gastos = st.multiselect(
      ...
      categorias_disponiveis,
      default=[],  # Inicia vazio (nada aparece até selecionar)
  ...
  if not st.session_state.historico_gastos.empty:
    ...
    # LÓGICA: Se todos estiverem selecionados OU nenhum estiver selecionado, não exibe nada.
    if (
        len(filtro_cat_gastos) == len(categorias_disponiveis)
        or len(filtro_cat_gastos) == 0
    ):
      df_filtrado_idx = []
    else:
    ...
    if len(df_filtrado_idx) > 0:
      ...
      st.info(
          "Nenhum gasto avulso exibido. Selecione categorias específicas no"
          " filtro acima."
      )
  else:
...
with tab3:
  ...
  # --- HISTÓRICO DE CARTÕES COM NOVA LÓGICA DE FILTRAGEM ---
  ...
  if (
    ...
    for i, nome_cartao in enumerate(nomes_cartoes_ativos):
      with tabs_cartoes[i]:
        ...
        # LÓGICA: Se todos ou nenhum estiverem selecionados, retorna dataframe vazio
        if (
            len(filtro_cat_cartoes) == len(categorias_disponiveis)
            or len(filtro_cat_cartoes) == 0
        ):
          df_cartao_atual = pd.DataFrame()
        else:
          df_cartao_atual = st.session_state.historico_cartoes[
              (st.session_state.historico_cartoes["Cartão"] == nome_cartao)
              & (
                  st.session_state.historico_cartoes["Classe/Categoria"].isin(
                      filtro_cat_cartoes
                  )
              )
          ]
        ...
        if not df_cartao_atual.empty:
          ...
          st.info(
              "Nenhum parcelamento exibido. Selecione categorias específicas"
              f" no filtro para o cartão **{nome_cartao}**."
          )
  else:
